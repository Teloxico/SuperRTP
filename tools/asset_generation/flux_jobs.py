...
engine, product, franchise or existing asset, and no image input is ever supplied.
...
def jobs_for(creative_id: str, family: str, width: int, height: int) -> list:
    ...
    if family in PART_COUNTS:
        ...
        for index in range(PART_COUNTS[family]):
            ...
            extra = {"expression": expressions[index % len(expressions)]} if prefix == "face" else {}
            add(f"{prefix}{index}", description, brief, size, **extra)
    elif family == "tiles":
        ...
        description, brief = describe(group, subject, variant)
        size = {"creature": (768, 768), "hero": (768, 768), "turnaround": (1024, 512), "object-turnaround": (1024, 512),
...
def plan_jobs(plan_entries) -> list:
    ...
    jobs.sort(key=lambda job: (rank.get(job.family, len(rank)), job.job_id))
