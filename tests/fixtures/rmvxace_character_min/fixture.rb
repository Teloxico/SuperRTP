# SuperRTP Clean-Room RGSS3 Character Test Harness
# Target: RPG Maker VX Ace (RGSS3)
...
# Assert RGSS3 default screen dimensions: 544x416 (Sanity check)
unless Graphics.width == 544 && Graphics.height == 416
  $stderr.puts "SUPERRTP_RGSS3_SCREEN_ERROR: Expected 544x416, got #{Graphics.width}x#{Graphics.height}"
  Process.exit!(3)
end
puts "SUPERRTP_RGSS3_SCREEN 544x416"
...
begin
  ...
  puts "SUPERRTP_RMVXACE_CHARACTER_LOADED #{char_bmp.width}x#{char_bmp.height}"

  unless char_bmp.width == 288 && char_bmp.height == 256
    $stderr.puts "SUPERRTP_RMVXACE_DIMENSION_ERROR: Expected 288x256, got #{char_bmp.width}x#{char_bmp.height}"
    Process.exit!(4)
  end
  ...
  puts "SUPERRTP_RMVXACE_RENDER_DONE"
  exit 0

rescue Errno::ENOENT => e
  $stderr.puts "SUPERRTP_RMVXACE_MISSING_ASSET: #{e.message}"
  $stderr.flush
  # Update Graphics for 45 frames to allow negative control capture of the blank pad
  45.times do
    Graphics.update
  end
  Process.exit!(1)
rescue => e
  $stderr.puts "SUPERRTP_RMVXACE_ERROR: #{e.class}: #{e.message}"
  Process.exit!(2)
end
